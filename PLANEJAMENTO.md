...
- 🔄 **Em andamento**: geração do gradle wrapper + primeiro build (`assembleDebug`) — pode revelar erros de compilação/versões de dependência a corrigir (checar APIs do NewPipeExtractor v0.24.3 e youtubedl-android 0.17.2 se falhar resolução)
- ⏭️ **Próximos passos**: (1) build verde; (2) `git init` + commit; (3) criar repo `Lumen-Connection/lumen-stream-mobile` via `gh` e push; (4) pendências do MVP: metadata Spotify real (1.10 — portar do desktop), teste em dispositivo real
