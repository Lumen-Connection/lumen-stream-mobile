...
- ✅ **Tag v0.1.0** publicada → workflow release gera APKs assinados no GitHub Releases
- ⏭️ **Backlog (fase 1.x)**: player em background/MediaSession, PiP, transcrição, F-Droid, mensagens de erro amigáveis (portar `friendly_error` do ytdlp_util.rs), NewPipe falhou na extração do YouTube no emulador (investigar; fallback cobre), fase 2 sync desktop
